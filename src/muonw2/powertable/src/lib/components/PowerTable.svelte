...
function checkBoxToggled(e: Event, index: number) {
    dispatch('checkBoxToggled', {event: e, data: pageData[index]});
}
                                                ...
                                                {#if specialInstructs.hasOwnProperty(instruct?.key)}
                                                    {#if instruct?.key === checkboxKey && options.checkboxColumn}
                                                        <td data-key={instruct.key}>
                                                            <input type="checkbox" bind:checked={data[record[dataIdKey]][checkboxKey]} on:change={(e)=>checkBoxToggled(e, record[dataIdKey])} />
                                                        </td>
                                                    {/if}
                                                {:else}
                                                    <td data-key={instruct.key}>
